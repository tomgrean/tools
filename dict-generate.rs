...
pub fn open(filename: &mut path::PathBuf) -> io::Result<()> {
    let mut file_con: Vec<u8>;
    {
        filename.set_extension("ext");
        let mut dict_file = fs::File::open(&filename)?;
    ...
    let lfidx = fs::File::create(&filename)?;
    filename.set_extension("dict");
    let lfdict = fs::File::create(&filename)?;
    let mut con = Parser {
    ...
    println!(
        "idxfilesize={}",
        con.fidx.seek(SeekFrom::Current(0)).unwrap()
    );
...
// .ext is editable file generated by dict-extract.rs.
// .idx is .idx file I will generate.
// .dict is .dict file I will generate.
// Example:
// dict-generate abc-dict
// will read abc-dict.ext, write abc-dict.idx and abc-dict.dict
fn main() -> io::Result<()> {
    ...
    let mut pbnameext = path::PathBuf::from(&fname);
