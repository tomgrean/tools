...
use std::{env, fmt, fs, io, num, path, str};
...
// I will extract .dict to .ext
// with the help of .idx file.
// the generated file .ext is used for edit.
// which will later be used to generate back
// to .idx and .dict
// Example:
// dict-extract abc-dict
// will read abc-dict.idx and abc-dict.dict, write abc-dict.ext
fn main() {
    ...
    let mut pbname = path::PathBuf::from(fname);
    pbname.set_extension("idx");
    let idxfile = Idx::open(&pbname).expect("open .idx file failed");

    //dict extraction
    pbname.set_extension("dict");
    let mut dictfile = fs::File::open(&pbname).expect("open .dict file failed");
    pbname.set_extension("ext");
    let mut writer = fs::File::create(&pbname).expect("open .ext file failed");
